...
（1）我们需要考虑不同的信任度，（2）最优化需要考虑所有可能的`u，v`对，而不仅仅是和观察数据相关的`u，v`对。显性反馈的矩阵分解优化时，对于missing data(未知评分)，是不会当做训练数据输入到模型的，优化时针对已知评分数据优化。而这里隐性反馈，是利用所有可能的`u`,`i`键值对，所以总的数据是`m*n`，其中`m`是用户数量，`n`是物品数量。这里没有所谓的`missing data`，因为假如`u`对`i`没有任何动作，我们就认为偏好值为0，只不过置信度较低而已。因此，通过最小化下面的损失函数来计算相关因素（`factors`）。
