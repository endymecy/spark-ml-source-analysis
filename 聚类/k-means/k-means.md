...
&emsp;&emsp;第1步初始化一个中心点，第2-6步计算出满足概率条件的多个候选中心点`C`，候选中心点的个数可能大于`k`个，所以通过第7-8步来处理。第7步给`C`中所有点赋予一个权重值<img src="http://www.forkosh.com/mathtex.cgi?{w}_{x}">，这个权重值表示距离`x`点最近的点得个数。
